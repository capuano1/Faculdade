...
not_runnable_pick_new:
	if (proc_is_preempted(p)) {
		...
		if (proc_is_runnable(p)) enqueue(p);
	}
 ...
 */
  ...
  if (!rdy_head[q]) {		/* add to empty queue */
	  ...
	  struct proc *aux1, *aux2;
	  aux1 = rdy_head[q];                       /* pointer to head of queue */
      aux2 = aux1->p_nextready;                     /* pointer to second process in queue */
      if (rp->p_cpu_time_left < aux1->p_cpu_time_left) {rdy_head[q] = rp; rp->p_nextready = aux1; goto rest;}                       /* in case rp has less time left than the first process in queue */
	  while(aux2 != NULL && rp->p_cpu_time_left > aux2->p_cpu_time_left) {aux1 = aux1->p_nextready; aux2 = aux2->p_nextready;}                      /* find where rp should go in queue based on its cpu time left */
      if (aux2 == NULL) {aux1->p_nextready = rp; rdy_tail[q] = rp; goto rest;}                        /* in case rp should go to the end of the queue, the tail */
	  aux1->p_nextready = rp;                       /* inserting rp in its place, if it's somewhere in the middle of the queue */
      rp->p_nextready = aux2;
  }
  
  rest:                     /* goto statement to get out of the else, in case rp should go to the first or last position on the queue */
