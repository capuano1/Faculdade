...
#include <math.h>
#include <string.h>
#include <C:\TDM-GCC-64\lib\gcc\x86_64-w64-mingw32\10.3.0\include\omp.h>
#include <C:\Program Files (x86)\Intel\oneAPI\mpi\2021.14\include\mpi.h>
...
void diff_eq(double **C, double **C_new, int first_row, int last_row, int rank, int size, MPI_Comm comm) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        double difmedio = 0.;
        #pragma omp target teams distribute parallel for collapse(2) reduction(+:difmedio)
        for (int i = 1; i <= last_row - first_row; i++) {
            for (int j = 1; j < N - 1; j++) {
...
void MPI(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        dados = fopen("FinalMPIOMPGPU-TestesComp.txt", "r");
        aux = fopen("aux6.txt", "w");
        int c;
        ...
        fclose(dados);
        fclose(aux);
        dados = fopen("FinalMPIOMPGPU-TestesComp.txt", "w");
        aux = fopen("aux6.txt", "r");
