...
void pista(){
  ...
  for(float j = 0; j<0.3;j+=0.2) {
    ...
    for(int i=0;i<n;i++){
      ...
      P.y = (1.0-t)*d.y+t*e.y;
      P.z = (1.0-t)*d.z+t*e.z;
      glNormal3f(0.0,P.y-j,0.0);
      glVertex3f(P.x,P.y-j,P.z-0.2);
      glNormal3f(P.x,P.y-j,P.z+0.2);
      glVertex3f(P.x,P.y-j,P.z+0.2);
      t+=tstep;
    }
    glEnd();
  }
  for(float j = -0.2; j<0.3;j+=0.4) {
    glBegin(GL_TRIANGLE_STRIP);
    t=0.0;
    for(int i=0;i<n;i++){
      a.x = (1.0-t)*P0.x+t*P1.x;
      ...
      P.z = (1.0-t)*d.z+t*e.z;
      glNormal3f(0.0,P.y-j,0.0);
      glVertex3f(P.x,P.y-0.2,P.z+j);
      glNormal3f(P.x,P.y+0.2,P.z+j);
      glVertex3f(P.x,P.y+0.2,P.z+j);
      t+=tstep;
    }
    glEnd();
...
int main(int argc, char *argv[]){
    ...
    glutCreateWindow("Entrega P2 Pt2");
